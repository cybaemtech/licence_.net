...
function VendorDetail() {
  ...
  const fetchVendorDetails = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${getApiBaseUrl()}/vendors/${encodeURIComponent(id ?? '')}`, {
        method: "GET",
        headers: {
  ...
  const { vendor, licenses, stats } = data;
      ...
      <div className="bg-white dark:bg-dark-800 rounded-lg shadow-sm p-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Vendor since {format(new Date(vendor.created_at), "MMM dd, yyyy")}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 border-t dark:border-dark-600 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {(vendor.currency_name || vendor.currency_code) && (
              <div className="space-y-1">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400">
                  Currency:
                </p>
                <p className="text-gray-900 dark:text-white">
                  {vendor.currency_name || vendor.currency_code}
                  {vendor.currency_symbol && ` (${vendor.currency_symbol})`}
                </p>
              </div>
            )}
            {vendor.mode_of_payment && (
              <div className="space-y-1">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400">
                  Mode of Payment:
                </p>
        ...
        <div className="bg-white dark:bg-dark-800 rounded-lg shadow-sm p-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Total Cost {vendor.currency_code && vendor.currency_code !== 'INR' && `(${vendor.currency_symbol || vendor.currency_code})`}
              </p>
              <div className="text-2xl font-bold text-gray-900 dark:text-white mt-1">
                {vendor.currency_symbol || (vendor.currency_code && vendor.currency_code !== 'INR' ? vendor.currency_code : '₹')}
            ...
            <tbody className="bg-white dark:bg-dark-800 divide-y divide-gray-200 dark:divide-dark-600">
              {licenses.length === 0 ? (
                <tr>
                  ...
                  >
                    No licenses found for this vendor
                  </td>
                </tr>
              ) : (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <SimpleCurrencyDisplay 
                        ...
                        preferredCurrency={vendor.currency_code || 'INR'}
                        className="text-sm"
                      />
                    </td>
                    <td className="px-4 py-3">
